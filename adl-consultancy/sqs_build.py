...
""", encoding="utf-8")
    ...
    custom_css = make_custom_css()

    # ---- 2. BODY END: CSS + sprite + header + footer + scripts -------------
...
<!-- ---------------------------------------------------------------------
     The Custom CSS is included here so this one file is self-sufficient.
     Caveat worth knowing: this is near the end of <body>, so it applies after
     the browser has already painted. Squarespace's own header can therefore be
     visible for a moment before it is hidden. If you see that flicker and it
     bothers you, ALSO paste 3-custom-css.css into Design > Custom CSS — that
     loads in <head> and applies before the first paint. Having it in both
     places is harmless.
     ------------------------------------------------------------------ -->
<style>
{custom_css}
